...
<section class="container landing-section">
    ...
    <article class="card landing-card checkout-card">
        ...
        <p class="muted">Meios habilitados: PIX, cartão de crédito e cartão de débito.</p>
        <div class="onboarding-summary-grid">
            ...
            <div><strong>Status do pagamento:</strong> <?= e($statusPagamento) ?></div>
            <div><strong>Status da assinatura:</strong> <?= e($statusAssinatura) ?></div>
        </div>

        <?php if ($statusPagamento === 'APROVADO'): ?>
            <p class="muted">Pagamento já aprovado. O acesso pode ser realizado pelo login institucional.</p>
            <a class="button" href="<?= e(url('/acessar-plataforma')) ?>">Acessar plataforma</a>
        <?php else: ?>
            <?php if ($checkoutUrl !== ''): ?>
                <p class="muted">Se o checkout externo estiver disponível, conclua por ele e depois retorne para confirmar.</p>
            ...
            <?php if ($checkoutMode === 'fallback_local'): ?>
                <form method="post" action="<?= e(url('/checkout/confirmar')) ?>" data-guard-submit="true" class="mt-1">
                    ...
                    <button type="submit" class="button" data-submit-label="Confirmar pagamento e ativar a assinatura">
                        <span class="button-text">Confirmar pagamento e ativar a assinatura</span>
                        <span class="button-loading" hidden>Processando...</span>
                    </button>
                </form>
            <?php else: ?>
                <p class="muted mt-1">A ativação ocorre automaticamente após confirmação de pagamento pelo Mercado Pago (retorno/webhook).</p>
            <?php endif; ?>
        <?php endif; ?>
    </article>
</section>
